...
const IPAS = root.IPAS || (root.IPAS = {});
...
async function analyzeIPA(arrayBuffer, fileMeta, opts) {
    ...
    const { Rules, ATS, MachO, Provisioning, Plist, Entropy } = IPAS;
    ...
    // 1. Standard case: Payload/Name.app/ — anywhere in the path, not just at the
    //    absolute root, so a zip with an extra wrapping folder (common when an IPA
    //    gets re-zipped by a tool, or downloaded and re-archived) still resolves.
    //    Casing is preserved from the actual entry instead of hardcoded, since some
    //    IPAs produced by non-Xcode tooling use lowercase "payload/".
    for (const path of allPaths) {
        const m = path.match(/(^|\/)([Pp]ayload\/[^/]+\.app\/)/);
        if (m) { appPath = path.slice(0, m.index + m[1].length) + m[2]; break; }
    }

    // 2. Fallback: no "Payload" folder at all — some tools zip the .app directly
    //    without the Payload wrapper. Look for a *.app/ directory that directly
    //    contains an Info.plist, so we don't grab a nested app extension by mistake.
    if (!appPath) {
        const appDirs = new Set();
        for (const path of allPaths) {
            const m = path.match(/(^|\/)([^/]+\.app\/)/);
            if (m) appDirs.add(path.slice(0, m.index + m[1].length) + m[2]);
        }
        for (const dir of appDirs) {
            if (zip.file(dir + 'Info.plist')) { appPath = dir; break; }
        }
        if (!appPath && appDirs.size > 0) appPath = [...appDirs][0];
    }

    if (!appPath) {
        const sample = allPaths.slice(0, 8).join(', ') || '(archive appears empty)';
        throw new Error(
            'No .app bundle found in IPA. Expected a Payload/Name.app/ folder inside the zip. ' +
            'Top-level entries seen: ' + sample +
            (allPaths.length > 8 ? ', …' : '') +
            '. This usually means the file isn\'t a real .ipa export (e.g. a plain zip of ' +
            'source files, a corrupted download, or a DRM-protected App Store binary).'
        );
    }
        ...
        if (provisioning && provisioning.meta) {
            ...
            if (provisioning.meta.getTaskAllow) {
                findings.push({
                    ...
                    cwe: 'CWE-489', owasp: 'M7', masvs: 'RESILIENCE-3', mitre: 'T1629',
                    category: 'provisioning',
                });
            }
        }
    ...
    if (specialFiles.databases.length > 0) {
        findings.push({
            ...
            cwe: 'CWE-312', owasp: 'M2', masvs: 'STORAGE-14', mitre: 'T1409',
            category: 'storage',
        });
    }
    if (specialFiles.certificates.length > 0) {
        findings.push({
            ...
            cwe: 'CWE-321', masvs: 'CRYPTO-1', mitre: 'T1634',
            category: 'crypto',
        });
    }
